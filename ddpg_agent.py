...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents=5, freeze_agent=False, freeze_critic=False, nn_deepth=4):
        ...
        # Noise process
        self.noise = OUNoise(action_size, random_seed, num_agents=num_agents)
...
class OUNoise:
    ...
    def update_noise_scalar(self, theta=None, scalar=None):
    ...
    def sample(self):
        ...
        #x = self.state
        #dx = self.theta * (self.mu - x) + self.sigma * np.random.sample((self.num_agents, self.size))
        #self.state = x + dx
        #return self.state
        return self.theta * np.random.normal(0, self.sigma, (self.num_agents, self.size))
