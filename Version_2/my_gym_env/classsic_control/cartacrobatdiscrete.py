...
class CartAcrobatDiscreteEnv(gym.Env):   #CartDoublePoleEnv
    ...
    def get_reward(self):
        x_next, x_dot_next, theta_1_next, theta_dot_1_next, theta_2_next, theta_dot_2_next = self.state
        if abs(theta_1_next) < np.deg2rad(15) and abs(theta_2_next) < np.deg2rad(15):
            return 1.0
        else:
            return 0.0
